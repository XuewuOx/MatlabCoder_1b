...
% % gaussian fitting
 [sigma, mu, A] = gfit(dataMS,dataUV_mean,0.2);
%    % % plot gaussian fitting results
    fprintf('Gaussian fit OK.\n optimal postion(step) is %04d (mu=%f, sigma=%f, A=%f)\n',...
         round(mu),mu, sigma, A);
    y_gf=A*exp(-(dataMS-mu).^2/(2*sigma^2));
    plot(dataMS,y_gf,'.-m');
    stem(round(mu),A,'sr');
    title(sprintf('IR/UV vs steps \n opt=%d',round(mu)), 'fontsize', 14);
